...
- `list_vents_by_room_temperature` (generic vent filtering by temperature + open/closed state)
...
npx -y mcporter call --server flair --tool list_vents_by_room_temperature --args '{"temperature_operator":"gte","threshold_temp_f":74,"vent_state":"closed"}' --output json
...
`list_vents_by_room_temperature` is the preferred generic call for vent state + temperature filtering.
